...
| Priory priest-allocation Google Sheet | Optional implemented integration | Supplies mission-specific priest availability windows while all detailed appointments stay in the mission database. The app caches the last verified schedule, warns on source failures or changed allocations, and writes only privacy-safe availability requests. | Priory-owned protected Sheet; permanent Priest and Mission IDs; mission-specific service account; configuration in Chapel Settings; encrypted deployment credentials; schema and permission contract in `docs/integrations/priory-priest-schedule.md`. |
