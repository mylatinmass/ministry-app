...
const ministryLeaveMigration = await read(
  "migrations/20260901_01_add_ministry_leave_assignment_restore.sql",
)
const ministryLeaveRestore = await read(
  "src/server/legacy/helper/ministry-leave-assignments.js",
)
...
assert.match(familyProfiles, /action === "leave_ministry"/)
assert.match(familyProfiles, /request_source, previous_assignment_status/)
assert.match(familyProfiles, /membership\.left/)
assert.match(ministryLeaveMigration, /request_source STRING NOT NULL DEFAULT 'member'/)
assert.match(ministryLeaveMigration, /previous_assignment_status STRING NULL/)
assert.match(ministryLeaveRestore, /request_source = 'ministry_leave'/)
assert.match(ministryLeaveRestore, /status = 'cancelled'/)
...
assert.match(availabilityComponent, /Unavailable Dates/)
assert.match(availabilityComponent, /Unavailable Date Ranges/)
assert.match(availabilityComponent, /availability-edit-date/)
assert.match(availabilityComponent, /availability-edit-range/)
assert.match(availabilityComponent, /availability-edit-rule/)
assert.match(schedulingAvailability, /update_availability_rule/)
assert.match(schedulingAvailability, /update_block/)
...
assert.match(ministryMembersComponent, /aria-pressed=\{selectedMemberId === member\.id\}/)
assert.match(ministryMembersComponent, /Ministry member details/)
assert.match(ministryMembersComponent, /action: "set_member_groups"/)
...
assert.match(schedulingOrdo, /loadOrdoCatalog/)
assert.match(schedulingOrdo, /external_solemnity/)
assert.match(ordoReference, /isOrdoSource \? "1962 Ordo" : "Ordo source"/)
assert.match(ordoReference, /Sacristy page and setup notes/)
assert.match(ordoReference, /Mass being celebrated/)
...
assert.match(assignmentNotifications, /Daily Admin Staffing Report/)
assert.match(assignmentNotifications, /loadDailyAdminDetails/)
assert.match(assignmentNotifications, /Unfilled required positions/)
...
assert.match(managedProfileComponent, /Remove a parent link/)
assert.match(managedProfileComponent, /guardianIdToRemove/)
assert.match(managedProfileComponent, /data-guide-id="profile-child-edit"/)
assert.match(managedProfileComponent, /data-guide-id="profile-child-calendar-color-toggle"/)
assert.match(managedProfileComponent, /absolute left-0 top-full z-30["]?[^\n]*grid-cols-4/)
assert.match(managedProfileComponent, /data-guide-id="profile-child-guardian-toggle"/)
assert.match(managedProfileComponent, /data-guide-id="profile-child-leave-ministry"/)
assert.match(managedProfileComponent, /Are you sure you want to LEAVE/)
assert.match(managedProfileComponent, /DON'T LEAVE/)
